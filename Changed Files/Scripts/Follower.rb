...
#===============================================================================
Unused_Common_Event        = 2        # Common event should be blank, but reserved
Following_Activated_Switch = 35       # Switch should be reserved
Toggle_Following_Switch    = 36       # Switch should be reserved
Current_Following_Variable = 36       # Variable should be reserved
ItemWalk = 8                          # Variable should be reserved
Walking_Time_Variable = 9             # Variable should be reserved
Walking_Item_Variable = 10            # Variable should be reserved
Animation_Come_Out = 37               
Animation_Come_In = 38
Emo_Happy = 39
Emo_Normal = 40
Emo_Hate = 41
Emo_Poison= 42
...
#===============================================================================           
            ...
            items=[:MAXREVIVE,:MAXREVIVE] # This array can be edited and extended. Look at the one below for a guide
            random2=0
            loop do
...
#===============================================================================            
            ...
            items=[:POTION,:SUPERPOTION,:FULLRESTORE,:REVIVE,
                 :PPUP,:PPMAX,:RARECANDY,:REPEL,:MAXREPEL
            ]
            random2=0
            loop do
              random2=rand(items.length)
  ...
  def pbFollowEventAcrossMaps(leader,follower,instant=false,leaderIsTrueLeader=true)
    ...
    if areConnected
      ...
      for i in 0...facings.length
        ...
        if !passable && $PokemonMap.bridge>0
          passable = pbIsBridge?($MapFactory.getTerrainTag(tile[0],tile[1],tile[2]))
          #passable = PBTerrain.isBridge?($MapFactory.getTerrainTag(tile[0],tile[1],tile[2]))
        elsif passable && !$PokemonGlobal.surfing && $PokemonMap.bridge==0       
          passable=!pbIsWaterTag?($MapFactory.getTerrainTag(tile[0],tile[1],tile[2]))
        end
...
class Scene_Map
  ...
  def update
    ...
    if $game_switches[Following_Activated_Switch] && $Trainer.party.length>0
      if Input.trigger?(Input::C) && !$game_switches[46] # try to talk to pokemon
