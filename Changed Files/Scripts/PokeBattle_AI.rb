...
class PokeBattle_Battle
  ...
  def pbGetNaturePowerMoveID()
    moveid = 0
    case $fefieldeffect
      when 1
        moveid=getConst(PBMoves,:THUNDERBOLT) || 0
      when 2
        moveid=getConst(PBMoves,:ENERGYBALL) || 0
      when 3
        moveid=getConst(PBMoves,:MISTBALL) || 0
      when 4
        moveid=getConst(PBMoves,:DARKPULSE) || 0
      when 5
        moveid=getConst(PBMoves,:ANCIENTPOWER) || 0
      when 6
        moveid=getConst(PBMoves,:ACROBATICS) || 0
      when 7
        moveid=getConst(PBMoves,:FLAMETHROWER) || 0
      when 8
        moveid=getConst(PBMoves,:MUDDYWATER) || 0
      when 9
        moveid=getConst(PBMoves,:AURORABEAM) || 0
      when 10
        moveid=getConst(PBMoves,:ACID) || 0
      when 11
        moveid=getConst(PBMoves,:ACIDSPRAY) || 0
      when 12
        moveid=getConst(PBMoves,:SANDTOMB) || 0
      when 13
        moveid=getConst(PBMoves,:ICEBEAM) || 0
      when 14
        moveid=getConst(PBMoves,:ROCKSMASH) || 0
      when 15
        moveid=getConst(PBMoves,:WOODHAMMER) || 0
      when 16
        moveid=getConst(PBMoves,:ERUPTION) || 0
      when 17
        moveid=getConst(PBMoves,:GEARGRIND) || 0
      when 18
        moveid=getConst(PBMoves,:DISCHARGE) || 0
      when 19
        moveid=getConst(PBMoves,:GUNKSHOT) || 0
      when 20
        moveid=getConst(PBMoves,:MEDITATE) || 0
      when 21
        moveid=getConst(PBMoves,:WHIRLPOOL) || 0
      when 22
        moveid=getConst(PBMoves,:WATERPULSE) || 0
      when 23
        moveid=getConst(PBMoves,:ROCKTOMB) || 0
      when 24
        moveid=getConst(PBMoves,:METRONOME) || 0
      when 25
        moveid=getConst(PBMoves,:POWERGEM) || 0
      when 26
        moveid=getConst(PBMoves,:SLUDGEWAVE) || 0
      when 27
        moveid=getConst(PBMoves,:ROCKSLIDE) || 0
      when 28
        moveid=getConst(PBMoves,:AVALANCHE) || 0
      when 29
        moveid=getConst(PBMoves,:JUDGMENT) || 0
      when 30
        moveid=getConst(PBMoves,:MIRRORSHOT) || 0
      when 31
        moveid=getConst(PBMoves,:SECRETSWORD) || 0
      when 32
        moveid=getConst(PBMoves,:DRAGONPULSE) || 0
      when 33
        if $fecounter == 4
          moveid=getConst(PBMoves,:PETALBLIZZARD) || 0          
        else
          moveid=getConst(PBMoves,:GROWTH) || 0
        end
      when 34
        moveid=getConst(PBMoves,:MOONBLAST) || 0
      when 35
        moveid=getConst(PBMoves,:SPACIALREND) || 0
      when 36
        moveid=getConst(PBMoves,:TRICKROOM) || 0
      when 37
        moveid=getConst(PBMoves,:PSYCHIC) || 0
      when 38
        moveid=getConst(PBMoves,:DARKPULSE) || 0
      when 39
        moveid=getConst(PBMoves,:ICEBEAM) || 0
      when 40
        moveid=getConst(PBMoves,:PHANTOMFORCE) || 0
      when 41
        moveid=getConst(PBMoves,:GUNKSHOT) || 0
      when 42
        moveid=getConst(PBMoves,:DAZZLINGGLEAM) || 0
      when 43
        moveid=getConst(PBMoves,:SKYATTACK) || 0
      else
        moveid=getConst(PBMoves,:TRIATTACK) || 0
    end
    return moveid
  end
  
  def pbGetNaturePowerMoveScore(move,attacker,opponent,skill)
    score = 0
    case $fefieldeffect
      when 1
...
#### AI Fix - Powder Moves on Grass types
    if opponent.pbHasType?(:GRASS)
      if move.id == PBMoves::COTTONSPORE || move.id == PBMoves::POISONPOWDER ||
         move.id == PBMoves::POWDER || move.id == PBMoves::RAGEPOWDER ||
         move.id == PBMoves::SLEEPPOWDER || move.id == PBMoves::SPORE ||
         move.id == PBMoves::STUNSPORE
        score-=95
      end
      if skill>=PBTrainerAI.bestSkill
        if opponent.hasWorkingAbility(:EFFECTSPORE) || opponent.hasWorkingAbility(:OVERCOAT)
          score-=95
        end
      end
    end
    ...
    case move.function
      ...
      when 0x03
        if opponent.pbCanSleep?(false)
          ...
          if skill>=PBTrainerAI.bestSkill
            ...
            if (isConst?(@id,PBMoves,:SING) && $fefieldeffect == 6 && 
             !opponent.hasWorkingAbility(:SOUNDPROOF)) ||
             (isConst?(@id,PBMoves,:SPORE) && 
              !opponent.hasWorkingAbility(:OVERCOAT) &&
              !isConst?(opponent.type1,PBTypes,:GRASS) &&
              !isConst?(opponent.type2,PBTypes,:GRASS) &&
      ...
      when 0x3F
        if !@opponent.is_a?(Array)
          score+=attacker.stages[PBStats::SPATK]*10 #if @opponent.trainertype != PBTrainers::SANDSTREAMER
#          if @opponent.trainertype != PBTrainers::SANDSTREAMER
#            score+=40
#          end
        else
          score+=attacker.stages[PBStats::SPATK]*10
      ...
      when 0x65
        if skill>=PBTrainerAI.mediumSkill
          if opponent.ability==0 ||
             ...
             isConst?(opponent.ability,PBAbilities,:SHIFT)
            score-=90
          end
        end
        if skill>=PBTrainerAI.highSkill
      ...
      when 0x66
        if opponent.effects[PBEffects::Substitute]>0
          ...
          if attacker.ability==0 ||
             ...
             isConst?(attacker.ability,PBAbilities,:SHIFT) ||
      ...
      when 0x9D
        score-=90 if @field.effects[PBEffects::MudSport]
      when 0x9E
        score-=90 if @field.effects[PBEffects::WaterSport]
      when 0x9F
      when 0xA0
      when 0xA1
      ...
      when 0xB2
      when 0xB3
        moveid = pbGetNaturePowerMoveID()
        move = PokeBattle_Move.pbFromPBMove(@battle,PBMove.new(moveid)) # PokeBattle_Move object of the move
        naturescore = pbGetNaturePowerMoveScore(move,attacker,opponent,skill)
        score += naturescore
      when 0xB4
        if attacker.status==PBStatuses::SLEEP
...
#            if !@opponent.is_a?(Array)
#              if @opponent.trainertype==PBTrainers::ENTOMOLOGIST
#                score+=200
#              end
#            end
          end
      ...
      when 0x15B
        if attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0 ||
           (attacker.pbOwnSide.effects[PBEffects::Reflect]>0 && attacker.pbOwnSide.effects[PBEffects::LightScreen]>0)
          score-=90
        elsif attacker.pbOwnSide.effects[PBEffects::AuroraVeil]==0 && skill>=PBTrainerAI.mediumSkill
          score+=60
        else
          score+=30
        end
      ...
      when 0x15D
      when 0x15E
        score-=90 if !attacker.hasType?(:FIRE)
      when 0x15F
      when 0x160
      when 0x161
        if attacker.turncount!=0
      ...
      when 0x162
        if attacker.pbIsOpposing?(opponent.index)
          ...
          if $fefieldeffect == 2 || $fefieldeffect == 31 || 
            ...
            score+=20 if opponent.hp<(opponent.totalhp/3) &&
                       opponent.effects[PBEffects::Substitute]==0
          end
        end
      when 0x163
        if attacker.pbTooHigh?(PBStats::ATTACK) &&
           attacker.pbTooHigh?(PBStats::SPATK) &&
           !attacker.pbPartner.isFainted? &&
           attacker.pbPartner.pbTooHigh?(PBStats::ATTACK) &&
           attacker.pbPartner.pbTooHigh?(PBStats::SPATK)
          score-=90
        else
          score-=attacker.stages[PBStats::ATTACK]*10
          score-=attacker.stages[PBStats::SPATK]*10
          if !attacker.pbPartner.isFainted?
            score-=attacker.pbPartner.stages[PBStats::ATTACK]*10
            score-=attacker.pbPartner.stages[PBStats::SPATK]*10
          end
        end
      when 0x164
      when 0x165
        ...
        if attacker.hp==attacker.totalhp
          ...
          case pbWeather
            when PBWeather::SANDSTORM
              score+=30
            when PBWeather::RAINDANCE, PBWeather::SUNNYDAY, PBWeather::HAIL
              score-=30
          end
          score+=50
          score-=(attacker.hp*100/attacker.totalhp)
      ...
      when 0x16F
        if skill>=PBTrainerAI.mediumSkill
          aspd=attacker.speed
          ospd=opponent.speed
          if aspd>=ospd && @trickroom==0
            score-=80
          elsif aspd>=ospd && @trickroom>0
            score+=(aspd-ospd)
          else
            score+=(ospd-aspd)
          end
      ...
      when 0x200
      when 0x201
        score+=20 if opponent.pbOwnSide.effects[PBEffects::StickyWeb]==false
      when 0x202
      when 0x203
      when 0x204
...
#### KUROTSUNE - 029 - START        
           ...
           (!attacker.hasWorkingAbility(:MOLDBREAKER) &&
...
#### KUROTSUNE - 029 - END
      if score != 0
        ...
        if move.pbTwoTurnAttack(attacker,true) || move.function==0xC2 # Hyper Beam
          basedamage*=2   # Not halved because semi-invulnerable during use or hits first turn
          basedamage/=3
        end
...
#        score=score*basedamage/[realBaseDamage,1].max
        if $INTERNAL
          PBDebug.log(sprintf("%s: damage (base=>percentage) %d=>%d",PBMoves.getName(move.id),realBaseDamage,basedamage))
  ...
  def pbBetterBaseDamage(move,attacker,opponent,skill,basedamage)
    ...
    case move.function
      ...
      when 0x171 # Stomping Tantrum
        basedamage*=2 if attacker.effects[PBEffects::Tantrum]
      when 0x202 # Galestrike
        n=(48*attacker.hp/attacker.totalhp).floor
        basedamage=50
        basedamage=60 if n<33
        basedamage=65 if n<17
        basedamage=100 if n<10
        basedamage=110 if n<5
        basedamage=130 if n<2
...
################################################################################
 ...
 # Consider boosts- if Pokemon can sweep, don't switch out.
    ...
    if shouldswitch
      ...
      party=pbParty(index)
      for i in 0...party.length
        next if i == party.length-1 && $game_switches[59]
