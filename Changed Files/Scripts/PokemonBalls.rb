...
BallHandlers::ModifyCatchRate.add(:STEAMBALL,proc{|ball,catchRate,battle,battler|
   catchRate*=3 if battler.pbHasType?(:FIRE) || battler.pbHasType?(:WATER)
   next catchRate
})

BallHandlers::ModifyCatchRate.add(:MINERALBALL,proc{|ball,catchRate,battle,battler|
   catchRate*=3 if battler.pbHasType?(:ROCK) || battler.pbHasType?(:GROUND) || battler.pbHasType?(:STEEL)
   next catchRate
})
...
BallHandlers::ModifyCatchRate.add(:DUSKBALL,proc{|ball,catchRate,battle,battler|
   catchRate*=7/2 if (PBDayNight.isNight?(pbGetTimeNow) || $game_switches[40])
   next catchRate
})
