...
def pbDayCareGenerateEgg
  ...
  egg.form = mainparent.form unless egg.species == 479 # New form inheriting
...
#  end
 ...
 # end
  ...
  for i in 0...2
    ...
    if isConst?(parent.item,PBItems,:POWERWEIGHT)
      ivinherit[i]=PBStats::HP 
      powercount+=1
    end  
    if isConst?(parent.item,PBItems,:POWERBRACER)
      ivinherit[i]=PBStats::ATTACK 
      powercount+=1
    end  
    if isConst?(parent.item,PBItems,:POWERBELT)
      ivinherit[i]=PBStats::DEFENSE 
      powercount+=1
    end  
    if isConst?(parent.item,PBItems,:POWERANKLET)
      ivinherit[i]=PBStats::SPEED 
      powercount+=1
    end  
    if isConst?(parent.item,PBItems,:POWERLENS)
      ivinherit[i]=PBStats::SPATK 
      powercount+=1
    end  
    if isConst?(parent.item,PBItems,:POWERBAND)
...
# Destiny Knot and Inheriting IVs
  destiny=false
  knot0=isConst?(mother.item,PBItems,:DESTINYKNOT)
  knot1=isConst?(father.item,PBItems,:DESTINYKNOT)
...
#### KUROTSUNE - 031 - END  
 ...
 # end
  ...
  if ditto0 || ditto1
     !isConst?(pbBallTypeToBall(father.ballused),PBItems,:MASTERBALL) &&
     !isConst?(pbBallTypeToBall(father.ballused),PBItems,:CHERISHBALL)
