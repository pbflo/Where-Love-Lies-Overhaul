...
def pbIsGrassTag?(tag)
  $fetag = 0
  $fetag = 1 if tag==PBTerrain::PokePuddle
...
def pbHasReflections?(tag)
  return tag==PBTerrain::StillWater ||
         tag==PBTerrain::Puddle
end

def pbIsDoubleWildBattle?(tag)
...
def pbBattleAnimation(battletype=0,bgm=nil,trainerid=-1,trainername="")
#### SARDINES - v17 - START
...
def pbDoubleWildBattle(species1,level1,species2,level2,variable=nil,canescape=true,canlose=false)
  ...
  pbPrepareBattle(battle)
  decision=0
  pbBattleAnimation(2,pbGetWildBattleBGM(species1)) { 
    pbSceneStandby {
      decision=battle.pbStartBattle(canlose)
    }
    for i in $Trainer.party; (i.makeUnmega rescue nil); end
...
def pbBattleOnStepTaken(repel=false)
  ...
  if $PokemonEncounters.pbCanEncounter?(encounter,repel)
    ...
    if !$PokemonTemp.forceSingleBattle && ($PokemonGlobal.partner ||
       ($Trainer.ablePokemonCount>1 && pbIsDoubleWildBattle?(pbGetTerrainTag) && rand(100)<30))
...
def Kernel.pbUpdateVehicle
  ...
  if meta
    if $PokemonGlobal.diving
      ...
      $game_player.character_name=pbGetPlayerCharset(meta,3) # Lava surfing graphic
    else
...
def Kernel.pbMountBike
  ...
  if $game_switches[999]
       Kernel.pbMessage(_INTL("You can't bike while riding on a Pokemon!"))
  else
