  ...
  def refresh
    ...
    if @refreshBitmap
      ...
      if @isContinue
        ...
        #textpos.push([_INTL("Pokémon Seen:"),16*2,72*2,0,Color.new(232,232,232),Color.new(136,136,136)])
        ...
        textpos.push([_INTL("Time:"),16*2,72*2,0,Color.new(232,232,232),Color.new(136,136,136)])
        hour = @totalsec / 60 / 60
        min = @totalsec / 60 % 60
        if hour>0
          #textpos.push([_INTL("{1}h {2}m",hour,min),103*2,88*2,1,Color.new(232,232,232),Color.new(136,136,136)])
          textpos.push([_INTL("{1}h {2}m",hour,min),103*2,72*2,1,Color.new(232,232,232),Color.new(136,136,136)])
        else
          #textpos.push([_INTL("{1}m",min),103*2,88*2,1,Color.new(232,232,232),Color.new(136,136,136)])
          textpos.push([_INTL("{1}m",min),103*2,72*2,1,Color.new(232,232,232),Color.new(136,136,136)])
        end
        if @trainer.isMale?
      ...
      begin
        ...
        while !success
          if safeExists?(RTP.getSaveFileName(trainer.lastSave))
            begin
              trainer, framecount, $game_system, $PokemonSystem, mapid=pbTryLoadFile(trainer.lastSave)
              haveBackup  = true
              showContinue= true
              success     = true
            rescue
