...
### DEPOSIT ####################################################################
      ...
      loop do
        ...
        if selected<0
          ...
          case command
            when 0 # Store
              if ($Trainer.party[selected].species==448 && $game_switches[128])
                pbDisplay(_INTL("You cannot deposit Lucario."))
              else
...
#### SARDINES - v17 - END
          ...
          case command
            when 0 # Move/Shift/Place
              if @heldpkmn && pokemon
                if (selected[0]==-1)
                  if ($Trainer.party[selected[1]].species==448 && $game_switches[128])
                    pbDisplay(_INTL("You cannot move Lucario."))
                  else
                    pbSwap(selected)
                  end
                else
