...
def pbTrainerName(name=nil,justName=false)
  ...
  def self.pbStoreItem(items,maxsize,maxPerSlot,item,qty,sorting=false)
    ...
    for i in 0...maxsize 
      ...
      if !itemslot     
        ...
        qty-=items[i][1]
        if sorting && POCKETAUTOSORT[$ItemData[item][ITEMPOCKET]]
          ...
          elsif $ItemData[item][ITEMPOCKET]==5
           items.sort!
          end         
        end
        return true if qty==0
...
def pbLoadPokemonBitmapSpecies(pokemon, species, back=false)
  ...
  if pokemon.isEgg?
    formnumber = pokemon.form rescue 0
    if formnumber!=0
      formmodifier = "_"+formnumber.to_s
    else
      formmodifier=""
    end    
    bitmapFileName=sprintf("Graphics/Battlers/%segg%s",getConstantName(PBSpecies,species),formmodifier) rescue nil
    if !pbResolveBitmap(bitmapFileName)
      bitmapFileName=sprintf("Graphics/Battlers/%segg",getConstantName(PBSpecies,species)) rescue nil
      if !pbResolveBitmap(bitmapFileName) 
        bitmapFileName=sprintf("Graphics/Battlers/%03degg%s",species,formmodifier)
        if !pbResolveBitmap(bitmapFileName)                
...
def pbMailBackFile(item)
  return nil if !item
  bitmapFileName=sprintf("Graphics/Pictures/Mail/mail%s",getConstantName(PBItems,item)) rescue nil
  if !pbResolveBitmap(bitmapFileName)
    bitmapFileName=sprintf("Graphics/Pictures/Mail/mail%03d",item)
  end
  return bitmapFileName
end
...
def pbPlayCryFirst(volume=90,pitch=nil)  
  pokemon = pbFirstAblePokemon(1)
  return if !pokemon
  if pokemon.is_a?(Numeric)
    pkmnwav=pbCryFile(pokemon)
    if pkmnwav
...
def pbRentalSingle
  $game_variables[14] = $Trainer.party
  $Trainer.party = []
...
def pbRentReturn
  $Trainer.party = $game_variables[14]
...
def pbAddSilent(pokemon,level=nil,seeform=true,ivs=[],ability=-1,moves=[],female=false,obtainText="",name="",ot="",shiny=-1)
  return false if !pokemon || !$Trainer || $Trainer.party.length>=6
  if pokemon.is_a?(String) || pokemon.is_a?(Symbol)
    pokemon=getID(PBSpecies,pokemon)
  end
