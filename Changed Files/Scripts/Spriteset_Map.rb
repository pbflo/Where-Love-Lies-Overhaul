...
# Clipping at Y
    ...
    while i<@rsprite.src_rect.height+Game_Map::TILEHEIGHT
      ...
      if @event.map.terrain_tag(@event.x,nextY)!=PBTerrain::StillWater && #The terrain types that will have reflections in them       
        @event.map.terrain_tag(@event.x,nextY)!=16   && 
        @event.map.terrain_tag(@event.x,nextY)!=18        
